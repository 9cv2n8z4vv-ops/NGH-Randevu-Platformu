...
function AppointmentActions({ item, customer, data, perform }: { item: any; customer: any; data: AdminData; perform?: (action: () => Promise<any>, success: string) => Promise<void> }) {
  ...
  return <div className="appointment-actions">
    ...
    <select aria-label="Ödeme yöntemi" value={paymentMethodId} onChange={(event) => setPaymentMethodId(event.target.value)}><option value="">Ödeme yöntemi</option>{data.paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select>
...
function Finance({ data, expenseForm, setExpenseForm, recurringForm, setRecurringForm, onExpense, onRecurring, perform, busy }: { data: AdminData; expenseForm: any; setExpenseForm: (value: any) => void; recurringForm: any; setRecurringForm: (value: any) => void; onExpense: (event: React.FormEvent<HTMLFormElement>) => void; onRecurring: (event: React.FormEvent<HTMLFormElement>) => void; perform: (action: () => Promise<any>, success: string) => Promise<void>; busy: boolean }) {
  ...
  const [manualIncome, setManualIncome] = useState({ description: "", amount: "", payment_method_id: data.paymentMethods[0]?.id || "", staff_id: "", occurred_on: new Date().toISOString().slice(0,10) });
  ...
  return <>
    ...
    <div className="admin-two-column admin-manage-grid"><section className="admin-panel"><div className="admin-panel-heading"><div><span className="eyebrow"><span className="eyebrow-line" /> TAHSİLAT DEFTERİ</span><h2>Gelir raporu</h2></div></div>{data.incomes.slice(0,12).map((row) => { const staff = relation(row.staff); const method = relation(row.payment_methods); return <div className="admin-list-row" key={row.id}><span><strong>{row.description || "Randevu geliri"}{row.is_voided && <small> · İptal edildi</small>}</strong><small>{formatDate(String(row.occurred_at).slice(0,10))} · {staff?.full_name || "Genel"} · {method?.name || ""}</small></span><b className={row.is_voided ? "muted-amount" : ""}>{formatMoney(row.collected_amount, currencyFor(data.settings))}</b></div>; })}{!data.incomes.length && <p className="admin-empty">Tamamlanan randevular ve manuel gelirler burada görünür.</p>}</section>
      ...
      <form className="admin-panel admin-form" onSubmit={addManualIncome}><div className="admin-panel-heading"><div><span className="eyebrow"><span className="eyebrow-line" /> HIZLI TAHSİLAT</span><h2>Randevusuz gelir</h2></div></div><label>Açıklama<input required value={manualIncome.description} onChange={(e) => setManualIncome({ ...manualIncome, description: e.target.value })} placeholder="Ürün satışı" /></label><div className="admin-form-inline"><label>Tutar<input required type="number" min="0" step="0.01" value={manualIncome.amount} onChange={(e) => setManualIncome({ ...manualIncome, amount: e.target.value })} /></label><label>Tarih<input required type="date" value={manualIncome.occurred_on} onChange={(e) => setManualIncome({ ...manualIncome, occurred_on: e.target.value })} /></label></div><div className="admin-form-inline"><label>Ödeme yöntemi<select value={manualIncome.payment_method_id} onChange={(e) => setManualIncome({ ...manualIncome, payment_method_id: e.target.value })}><option value="">Belirtilmedi</option>{data.paymentMethods.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><label>Personel<select value={manualIncome.staff_id} onChange={(e) => setManualIncome({ ...manualIncome, staff_id: e.target.value })}><option value="">Genel</option>{data.staff.map((row) => <option key={row.id} value={row.id}>{row.full_name}</option>)}</select></label></div><button className="button button-dark"><Plus size={15} /> Geliri kaydet</button></form></div></div>
    <div className="admin-two-column admin-manage-grid"><section className="admin-panel"><div className="admin-panel-heading"><div><span className="eyebrow"><span className="eyebrow-line" /> ÖDEME YÖNTEMLERİ</span><h2>Yöntemleri yönetin</h2></div></div>{data.paymentMethods.map((row) => <div className="admin-list-row" key={row.id}><span><strong>{row.name}</strong><small>{row.kind === "cash" ? "Nakit" : row.kind === "card" ? "Kart" : "IBAN"}</small></span><button className="admin-text-button" onClick={() => void perform(() => db.from("payment_methods").update({ is_active: !row.is_active }).eq("id", row.id), row.is_active ? "Ödeme yöntemi gizlendi." : "Ödeme yöntemi açıldı.")}>{row.is_active ? "Aktif" : "Gizli"}</button></div>)}<form className="admin-inline-create" onSubmit={addPaymentMethod}><label>Yeni yöntem<input required value={paymentForm.name} onChange={(e) => setPaymentForm({ ...paymentForm, name: e.target.value })} /></label><select aria-label="Yeni ödeme yöntemi türü" value={paymentForm.kind} onChange={(e) => setPaymentForm({ ...paymentForm, kind: e.target.value })}><option value="cash">Nakit</option><option value="card">Kart</option><option value="iban">IBAN</option></select><button className="button button-outline"><Plus size={14} /> Ekle</button></form></section>
