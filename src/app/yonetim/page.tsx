...
export default async function AdminPage() {
  ...
  const [settings, appointments, services, categories, staff, paymentMethods, expenses, incomes, hours, closures, recurring, expenseCategories, customers, staffSchedule, staffTimeOff, staffServices, testimonials, gallery, faqs, auditLogs] = await Promise.all([
    ...
    supabase.from("income_transactions").select("id,source,occurred_at,expected_amount,collected_amount,description,is_voided,staff_id,staff(full_name),payment_methods(name),income_transaction_services(service_name_snapshot,amount)").gte("occurred_at", from).order("occurred_at", { ascending: false }).limit(100),
